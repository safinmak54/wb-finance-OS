...
import { listUnclassifiedAdminApi } from "@/lib/queries/transactions";
...
export default async function CashbookInboxPage({
  searchParams,
}: {
  ...
  const [rows, accounts, entities, rules, banks] = await Promise.all([
    listUnclassifiedAdminApi(supabase, { entity, codeToId }),
  ...
  const enrichedRows = rows.map((r) => {
    ...
    return {
      ...
      category: cashbookCategory(r.description),
    };
  });

  const sources = Array.from(new Set(rows.map((r) => r.source))).sort();

  // Only surface categories that actually occur, kept in canonical order.
  const present = new Set(enrichedRows.map((r) => r.category));
  const categories = CASHBOOK_CATEGORIES.filter((c) => present.has(c));
